import type { Interval, RankFunction, RankOptions } from "@ixfx/core";
import type { TimeoutPingOptions, TimeoutValueOptions } from "./from/types.js";
import type { ChunkOptions, DebounceOptions, FieldOptions, FilterPredicate, SingleFromArrayOptions, SplitOptions, SwitcherOptions, SyncOptions, ThrottleOptions } from "./ops/types.js";
import type { CombineLatestOptions, InitStreamOptions, PipeSet, Reactive, ReactiveOp, ReactiveOrSource, ReactivePingable, ReactiveStream, ReactiveWritable, RxValueTypeObject, RxValueTypes, Unsubscriber, WithValueOptions } from "./types.js";
...
export function run<TIn, TOut>(source: ReactiveOrSource<any>, ...ops: Array<ReactiveOp<any, any>>): Reactive<any> {
...
export const Ops = {
   ...
   * @returns
   */
  combineLatestToArray: <const T extends ReadonlyArray<ReactiveOrSource<any>>>(options: Partial<CombineLatestOptions> = {}): (sources: T) => Reactive<RxValueTypes<T>> => {
    return (sources: T) => {
  ...
  pipe: <TInput, TOutput>(...streams: Array<Reactive<any> & ReactiveWritable<any>>): (source: ReactiveOrSource<TInput>) => Reactive<unknown> => {
  ...
  split: <V>(options: Partial<SplitOptions> = {}): (source: ReactiveOrSource<V>) => Array<ReactiveStream<V>> => {
    return (source: ReactiveOrSource<V>) => {
  ...
  syncToArray: <const T extends ReadonlyArray<ReactiveOrSource<any>>>(options: Partial<SyncOptions> = {}): (reactiveSources: T) => Reactive<RxValueTypes<T>> => {
  ...
  tapOps: <In, Out>(...ops: Array<ReactiveOp<In, Out>>): (source: ReactiveOrSource<In>) => Reactive<Out> => {
    return (source: ReactiveOrSource<In>) => {
 ...
 * @param transform
 */
export function to<TA, TB>(a: Reactive<TA>, b: ReactiveWritable<TB>, transform?: (valueA: TA) => TB, closeBonA = false): Unsubscriber {
