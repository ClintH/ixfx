import type { Grid, GridBoundsLogic, GridCardinalDirection, GridCell, GridNeighbour, GridNeighbourMaybe, GridNeighbours } from "./types.js";
 ...
 * @return
 */
function isNeighbour(n: GridNeighbour | GridNeighbourMaybe | undefined): n is GridNeighbour {
  if (n === undefined)
    return false;
  if (n[1] === undefined)
    return false;
 ...
 */
export function neighbourList(grid: Grid, cell: GridCell, directions: readonly GridCardinalDirection[], bounds: GridBoundsLogic): readonly GridNeighbour[] {
 ...
 */
export function neighbours(grid: Grid, cell: GridCell, bounds: GridBoundsLogic = `undefined`, directions?: readonly GridCardinalDirection[]): GridNeighbours {
  const _directions = directions ?? allDirections;
  const points = _directions.map(c =>
    offset(grid, cell, getVectorFromCardinal(c), bounds),
  );
  return zipKeyValue<GridCell>(_directions, points) as GridNeighbours;
}