 ...
 */
export function isNull(p: Point): boolean {
  if (isPoint3d(p)) {
    if (p.z !== null)
      return false;
  }
 ...
 */
export function isNaN(p: Point): boolean {
  if (isPoint3d(p)) {
    if (!Number.isNaN(p.z))
      return false;
  }
...
export function pointTest(p: Point, name = `Point`, extraInfo = ``): Result<Point, string> {
  if (p === undefined) {
  ...
  if (typeof p !== `object`) {
    return errorResult(
      `'${name}' is type '${typeof p}'. Expected object.`,
      extraInfo,
    );
  }
  if (p.x === undefined) {
 ...
 */
export function guardNonZeroPoint(pt: Point | Point3d, name = `pt`) {
  guard(pt, name);
  resultThrow(
...
export function isPoint(p: number | unknown): p is Point {
  ...
  if (p === null)
    return false;
  if ((p as Point).x === undefined)
    return false;
  if ((p as Point).y === undefined)
    return false;
  return true;
}
 ...
 */
export function isPoint3d(p: Point | unknown): p is Point3d {
  if (p === undefined)
    return false;
  if (p === null)
    return false;
  if ((p as Point3d).x === undefined)
    return false;
  if ((p as Point3d).y === undefined)
    return false;
  if ((p as Point3d).z === undefined)
    return false;
  return true;
}
 ...
 */
export function isEmpty(p: Point): boolean {
  if (isPoint3d(p)) {
    if (p.z !== 0)
      return false;
  }
 ...
 */
export function isPlaceholder(p: Point): boolean {
  if (isPoint3d(p)) {
    if (!Number.isNaN(p.z))
      return false;
  }
