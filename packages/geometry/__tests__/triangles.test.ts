...
function isCloseTo(a: number, b: number, decimals = 0) {
...
test(`isosceles`, () => {
  ...
  expect(isCloseTo(Tis.height(t), 19.364, 3)).toBeTruthy()
  expect(isCloseTo(Tis.perimeter(t), 50, 3)).toBeTruthy()
  expect(isCloseTo(Tis.area(t), 96.824, 3)).toBeTruthy()
  expect(isCloseTo(Tis.apexAngle(t), degreeToRadian(28.955), 3)).toBeTruthy()
  expect(isCloseTo(Tis.circumcircle(t).radius, 10.327, 3)).toBeTruthy()
  expect(isCloseTo(Tis.incircle(t).radius, 3.872, 3)).toBeTruthy()
  ...
  expect(isCloseTo(medians[ 2 ], 19.364, 3)).toBeTruthy()
});
...
test(`rightAngle`, () => {
  ...
  expect(isCloseTo(Tra.height(t), 8.320, 3), Tra.height(t).toString()).toBeTruthy()
  expect(isCloseTo(Tra.perimeter(t), 43.027, 3)).toBeTruthy()
  ...
  expect(isCloseTo(Tra.oppositeFromAdjacent(1, 10), 15.57407, 5), Tra.oppositeFromAdjacent(1, 10).toString()).toBeTruthy()
  expect(isCloseTo(Tra.oppositeFromHypotenuse(1, 10), 8.41470, 5), Tra.oppositeFromHypotenuse(1, 10).toString()).toBeTruthy()

  expect(isCloseTo(Tra.adjacentFromHypotenuse(1, 10), 5.40302, 5)).toBeTruthy()
  expect(isCloseTo(Tra.adjacentFromOpposite(1, 10), 6.42092, 5)).toBeTruthy()

  expect(isCloseTo(Tra.hypotenuseFromOpposite(1, 10), 11.88395, 5)).toBeTruthy()
  expect(isCloseTo(Tra.hypotenuseFromAdjacent(1, 10), 18.50815, 5)).toBeTruthy()
});
