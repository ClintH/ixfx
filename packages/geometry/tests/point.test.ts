...
describe(`point`, () => {
  ...
  it(`from-string`, () => {
    ...
    expect(Points.fromString(`10,20,`)).toEqual({ x: 10, y: 20, z: Number.NaN });
    expect(Points.fromString(`10,a`)).toEqual({ x: 10, y: Number.NaN });
    expect(Points.fromString(`b,20`)).toEqual({ x: Number.NaN, y: 20 });
    expect(Points.fromString(`asdf`)).toEqual({ x: Number.NaN, y: Number.NaN });
  });

  it(`angleRadianCircle`, () => {
  ...
  it(`divide`, () => {
    ...
    // B contains NaN
    expect(() => divide({ x: 10, y: 5 }, Number.NaN, 2)).toThrow();
    expect(() => divide({ x: 10, y: 5 }, 2, Number.NaN)).toThrow();
  });
  ...
  it(`multiply`, () => {
    ...
    expect(() => Points.multiply({ x: 10, y: 5 }, Number.NaN, 2)).toThrow();
    expect(() => Points.multiply({ x: 10, y: 5 }, 2, Number.NaN)).toThrow();
  });
  ...
  it(`sum`, () => {
    ...
    expect(() => Points.sum(Number.NaN, 2, 0, 0)).toThrow();
    expect(() => Points.sum(1, Number.NaN, 0, 0)).toThrow();
    expect(() => Points.sum(1, 2, Number.NaN, 0)).toThrow();
    expect(() => Points.sum(1, 2, 0, Number.NaN)).toThrow();
  });
  ...
  it(`compare-rowwise`, () => {
    expect(Points.compareRowwise({ x: 5, y: 5 }, { x: 5, y: 5 })).toBe(0);
    // -1 if a is less than b
    expect(Points.compareRowwise({ x: 5, y: 4 }, { x: 5, y: 5 })).toBe(-1);
    expect(Points.compareRowwise({ x: 4, y: 5 }, { x: 5, y: 5 })).toBe(-1);

    // 1 if a is greater than b
    expect(Points.compareRowwise({ x: 5, y: 6 }, { x: 5, y: 5 })).toBe(1);
    expect(Points.compareRowwise({ x: 6, y: 5 }, { x: 5, y: 5 })).toBe(1);

    const points = [{ x: 3, y: 5 }, { x: 4, y: 5 }, { x: 10, y: 3 }, { x: 5, y: 5 }, { x: 7, y: 6 }];
    expect(points.sort(Points.compareRowwise)).toEqual([
      { x: 10, y: 3 },
      { x: 3, y: 5 },
      { x: 4, y: 5 },
      { x: 5, y: 5 },
  ...
  it(`getAsBounds`, () => {
    expect(Points.getAsBounds({ x: 10, y: 20 }, { x: 30, y: 40 })).toEqual({ topLeft: { x: 10, y: 20 }, bottomRight: { x: 30, y: 40 } });
    expect(Points.getAsBounds({ x: 30, y: 40 }, { x: 10, y: 20 })).toEqual({ topLeft: { x: 10, y: 20 }, bottomRight: { x: 30, y: 40 } });
    expect(Points.getAsBounds({ x: 30, y: 40 }, { x: 30, y: 20 })).toEqual({ topLeft: { x: 30, y: 20 }, bottomRight: { x: 30, y: 40 } });
    expect(Points.getAsBounds({ x: 10, y: 40 }, { x: 30, y: 40 })).toEqual({ topLeft: { x: 10, y: 40 }, bottomRight: { x: 30, y: 40 } });
    expect(Points.getAsBounds({ x: 30, y: 30 }, { x: 30, y: 30 })).toEqual({ topLeft: { x: 30, y: 30 }, bottomRight: { x: 30, y: 30 } });
  });
  ...
  it(`subtract`, () => {
    ...
    expect(() => Points.subtract(Number.NaN, 2, 0, 0)).toThrow();
    expect(() => Points.subtract(1, Number.NaN, 0, 0)).toThrow();
    expect(() => Points.subtract(1, 2, Number.NaN, 0)).toThrow();
    expect(() => Points.subtract(1, 2, 0, Number.NaN)).toThrow();
  });
