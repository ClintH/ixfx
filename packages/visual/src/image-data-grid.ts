 ...
 * @returns Grid
 */
export function grid(image: ImageData): Grids.UniformGrid {
 ...
 * @param image
 */
export function wrap(image: ImageData): Grids.GridWritable<Rgb8Bit> & Grids.GridReadable<Rgb8Bit> {
  return {
    ...grid(image),
    get: accessor(image),
    set: setter(image),
 ...
 * @param image
 */
export function accessor(image: ImageData): Grids.GridCellAccessor<Rgb8Bit> {
  const g = grid(image);
 ...
 * @param image
 */
export function setter(image: ImageData): Grids.GridCellSetter<Rgb> {
  const g = grid(image);
  const data = image.data;
 ...
 * @param image
 */
export function *byRow(image: ImageData): Generator<Array<Rgb8Bit | undefined>, void, unknown> {
  const a = accessor(image);
  const g = grid(image);
