import type { Colourish, Hsl, OkLch, Rgb } from "./types.js";

export function isHsl(v: any): v is Hsl {
  if (typeof v !== `object`)
    return false;
  if (!(`h` in v))
    return false;
...
export function isRgb(v: any): v is Rgb {
  if (typeof v !== `object`)
    return false;
  if (!(`r` in v))
    return false;
 ...
 * @returns
 */
export function tryParseObjectToRgb(v: any): Rgb | undefined {
  if (typeof v !== `object`)
...
export function tryParseObjectToHsl(v: any): Hsl | undefined {
...
export function isOkLch(v: any): v is OkLch {
  if (typeof v !== `object`)
    return false;
  if (!(`l` in v))
    return false;
...
export function isColourish(v: any): v is Colourish {
  if (typeof v === `string`)
    return true;
  if (typeof v !== `object`)
    return false;
