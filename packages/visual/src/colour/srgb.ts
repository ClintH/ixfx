...
export function withOpacity<T extends Rgb>(value: T, fn: (opacityScalar: number, value: T) => number): T {
  switch (value.unit) {
    case `8bit`:
      return {
        ...value,
...
export function fromCss(value: string, options: ParsingOptions<Rgb> = {}): Rgb {
  ...
  if (typeof cssDefinedHexColours[value] !== `undefined`)
    return fromHexString(cssDefinedHexColours[value] as string, scalar);
...
export function toCssString(rgb: Rgb): string {
  guard(rgb);
  switch (rgb.unit) {
    case `8bit`:
...
function toLibrary(rgb: Rgb): C.RGB {
...
export function to8bit(rgbOrString: Rgb | string): Rgb8Bit {
...
export function toScalar(rgbOrString: Rgb | Hsl | string): RgbScalar {
  if (typeof rgbOrString === `string`) {
...
export function guard(rgb: Rgb): void {
 ...
 */
export function changeLightness(rgb: Rgb, amount: Partial<{ pdelta: number; delta: number; fixed: number }>): Rgb {
  let newL = 0;
...
function scaleProperty(rgb: Rgb, value: number, property: `r` | `g` | `b` | `opacity`) {
  if (rgb.unit === `scalar`) {
    // 0..1 scale
    if (value > 1)
      value = 1;
 ...
 * @param colourB
 */
export function interpolator(colourA: Rgb | string, colourB: Rgb | string) {
