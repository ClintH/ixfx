import type { Colour, ColourInterpolationOpts, ColourInterpolator, Colourish, ColourSpaces, ColourStepOpts, ConvertDestinations, HslScalar, OkLchScalar, RgbScalar } from "./types.js";
 ...
 */
export function interpolator(colours: Colourish[], opts: Partial<ColourInterpolationOpts> = {}) {
  ...
  return (amt: number): Colour => {
    ...
    if (amt >= 1)
      return asTargetDestination.at(-1)!;
    // If we're at the beginning, return the first colour
    if (amt <= 0)
      return asTargetDestination.at(0)!;
 ...
 */
function interpolateInit(colours: Colourish[], destination: ColourSpaces = `hsl`): Array<Array<OkLchScalar | HslScalar | RgbScalar>> {
 ...
 */
export function cssLinearGradient(colours: Colourish[]) {
 ...
 */
export function interpolatorDual(colourA: Colourish, colourB: Colourish, options: Partial<ColourInterpolationOpts> = {}): (amount: number) => Colour {
  const space = options.space ?? `oklch`;
  const direction = options.direction ?? `shorter`;
 ...
 */
export function scale(colours: Colourish[], opts: Partial<ColourStepOpts> = {}): string[] {
  const direction = opts.direction ?? `shorter`;
