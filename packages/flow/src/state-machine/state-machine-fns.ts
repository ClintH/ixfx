 ...
 */
export function cloneState<V extends Transitions>(toClone: MachineState<V>): MachineState<V> {
 ...
 */
export function init<V extends Transitions>(stateMachine: Machine<V> | Transitions | TransitionsStrict, initialState?: StateNames<V>): MachineState<V> {
  ...
  return Object.freeze({
    value: state,
    visited: [],
    machine: Object.freeze(Object.fromEntries(transitions)) as any as Readonly<Record<StateNames<V>, ReadonlyArray<StateTargetStrict<V>>>>,
  });
}

export function reset<V extends Transitions>(sm: MachineState<V>): MachineState<V> {
  return init<V>(sm.machine as any);
}

export function validateMachine<V extends Transitions>(smOrTransitions: Machine<V> | Transitions | TransitionsStrict): [ machine: Machine<V> | undefined, msg: string ] {
 ...
 * @param sm
 */
export function isDone<V extends Transitions>(sm: MachineState<V>): boolean {
 ...
 * @param sm
 */
export function possibleTargets<V extends Transitions>(sm: MachineState<V>): ReadonlyArray<StateTargetStrict<V>> {
 ...
 * @param sm
 */
export function possible<V extends Transitions>(sm: MachineState<V>): Array<StateNames<V> | null> {
  const targets = possibleTargets(sm);
  return targets.map(v => v.state);
}

export function normaliseTargets<V extends Transitions>(targets:
  | StateTarget<V>
  | ReadonlyArray<StateTargetStrict<V>>
  | StateTargetStrict<V>): Array<StateTargetStrict<V>> | null | undefined {
...
function validateAndNormaliseTransitions(d: Transitions): Map<string, Array<StateTargetStrict<typeof d>>> | undefined {
 ...
 * @param state
 */
function validateMachineState<V extends Transitions>(state: MachineState<V>): void {
 ...
 * @param toState
 */
export function to<V extends Transitions>(sm: MachineState<V>, toState: StateNames<V>): MachineState<V> {
  validateMachineState(sm); // throws if not OK
  validateTransition(sm, toState); // throws if not OK
  return Object.freeze({
    value: toState,
    machine: sm.machine,
    visited: [...new Set(...(sm.visited as string[]), sm.value as string)],
  });
}

export function next<V extends Transitions>(sm: MachineState<V>): MachineState<V> {
  // validateMachineState(sm);
 ...
 * @param toState
 */
export function isValidTransition<V extends Transitions>(sm: MachineState<V>, toState: StateNames<V>): boolean {
  try {
...
export function validateTransition<V extends Transitions>(sm: MachineState<V>, toState: StateNames<V>): void {
  if ((toState as unknown) === null)
    throw new Error(`Cannot transition to null state`);
  if (typeof toState === `undefined`) {
    throw new TypeError(`Cannot transition to undefined state`);
  }
 ...
 */
export function fromList(...states: readonly string[]): Transitions {
  const t = {};
  ...
  for (let index = 0; index < states.length; index++) {
    ...
    (t as any)[s] = (index === states.length - 1) ? null : states[index + 1] as string;
  }
  return t;
}
 ...
 * @param states
 */
export function fromListBidirectional(...states: readonly string[]): Transitions {
  const t = {};
  if (!Array.isArray(states))
    throw new Error(`Expected array of strings`);
  if (states.length < 2)
  ...
  for (let index = 0; index < states.length; index++) {
    const v = (t as any)[states[index] as string] as string[];
    if (index === states.length - 1) {
      if (states.length > 1) {
        v.push(states[index - 1] as string);
      } else {
        (t as any)[states[index] as string] = null;
      }
    } else {
      v.push(states[index + 1] as string);
      if (index > 0)
