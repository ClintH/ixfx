...
test('basic', async () => {
  ...
  expect(result.value).toEqual({ name: 'Sue' });
});
...
test('count', async () => {
  ...
  await expect(retryFunction(fn, { limitAttempts: 0 })).rejects.toThrowError();
  await expect(retryFunction(fn, { limitAttempts: -1 })).rejects.toThrowError();
  await expect(retryFunction(fn, { limitAttempts: Number.NaN })).rejects.toThrowError();
