...
import { zipObjects } from '../../src/records/zip.js';
...
describe(`records/zip`, () => {
  describe(`zipObjects`, () => {
    test(`zips two arrays of same length`, () => {
      ...
      const result = [...zipObjects(a as any, b as any)];
    ...
    test(`throws for array length mismatch`, () => {
      const a = [{ id: 1 }];
      const b = [{ id: 1 }, { id: 2 }];
      expect(() => [...zipObjects(a as any, b as any)]).toThrow(/Array length mismatch/);
    });

    test(`throws for non-array first parameter`, () => {
      const b = [{ id: 1 }];
      expect(() => [...zipObjects({} as any, b)]).toThrow(/Value at index 0 is not an array as expected/);
    });

    test(`throws for non-array second parameter`, () => {
      const a = [{ id: 1 }];
      expect(() => [...zipObjects(a, {} as any)]).toThrow(/Value at index 1 is not an array as expected/);
    });
    ...
    test(`is iterable`, () => {
      const a = [{ a: 1 }];
      const b = [{ b: 2 }];
      const iterator = zipObjects(a as any, b as any);
