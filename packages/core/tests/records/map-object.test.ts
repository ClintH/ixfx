...
import { mapObjectShallow, mapObjectByObject } from '../../src/records/map-object.js';
...
test(`by-object`, () => {
  ...
  const r1 = mapObjectByObject(a, {
    // Should be ignored
    blah: () => 10,
    name: (value: string, context: string) => value.toUpperCase(),
  });
  expect(r1).toEqual({
    name: `JOHN`,
...
test('results', () => {
  ...
  // Test original was not changed
  expect(o2a).toEqual({ width: 100, height: 250, colour: 'red' });
});
