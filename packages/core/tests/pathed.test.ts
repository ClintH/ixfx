...
import { resultToValue } from '@ixfx/guards';
import { assert, expect, it } from 'vitest';
import { applyChanges, compareData, getField, getPaths, getPathsAndData, updateByPath } from '../src/pathed.js';

it(`get-paths-and-data`, () => {
  const d1 = {
    accel: { x: 1, y: 2, z: 3 },
    gyro: { x: 4, y: 5, z: 6 },
  };
...
it(`get-paths`, () => {
  ...
  const paths2 = [...getPaths(d, { onlyLeaves: true })];
  ...
  assert.sameMembers(d2R, [
    ...
    `profiles.0.name`,
    `profiles.0.animals`,
    `profiles.0.animals.0`,
    `profiles.0.animals.1`,
    `profiles.1`,
    `profiles.1.name`,
    `profiles.1.animals`,
    `profiles.1.animals.0`,
    `profiles.1.animals.1`,
  ]);
});
...
it(`compare - data - deep -with-parents`, () => {
  const t1 = {
    a: {
      b: {
        c: {
