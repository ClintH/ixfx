 ...
 * @returns
 */
export function numericComparer(a: number, b: number): CompareResult {
  // ✔️ Unit tested
  if (a === b)
    return 0;
  if (a > b)
...
export function jsComparer(x: any, y: any): CompareResult {
  // ✔️ Unit tested
 ...
 * @returns
 */
export function comparerInverse<V>(comparer: Comparer<V>): Comparer<V> {
  return (x: V, y: V) => {
