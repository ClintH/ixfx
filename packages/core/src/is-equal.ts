import { toStringDefault, toStringOrdered } from "./to-string.js";
 ...
 */
export function isEqualValueDefault<T>(a: T, b: T): boolean {
  // ✔ UNIT TESTED
  if (a === b)
 ...
 * @returns
 */
export function isEqualValuePartial(a: Record<string, unknown>, b: Record<string, unknown>, fieldComparer?: IsEqual<unknown>): boolean {
  if (typeof a !== `object`)
 ...
 */
export function isEqualValueIgnoreOrder<T>(a: T, b: T): boolean {
  // ✔ UNIT TESTED
  if (a === b)
    return true; // Object references are the same, or string values are the same
