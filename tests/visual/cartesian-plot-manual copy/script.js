...
window.addEventListener(`pointermove`, event => {
  ...
  const sourceRel = source.toRelPoint(raw);
  const regionRel = region.toRelPoint(raw, `independent`);
  const regionClamp = region.pointSourceToRegion(raw, true);
  const screenToValue = p.screenToValue(raw, true);
