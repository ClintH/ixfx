...
test('divideFn', t => {
  let f = Points.divideFn(100, 200);
  t.like(f(50, 100), {x: 0.5, y: 0.5});
  t.like(f({x: 50, y: 100}), {x: 0.5, y: 0.5});
  t.like(f([50, 100]), {x: 0.5, y: 0.5});

  f = Points.divideFn([100, 200]);
  t.like(f(50, 100), {x: 0.5, y: 0.5});
  t.like(f({x: 50, y: 100}), {x: 0.5, y: 0.5});
  t.like(f([50, 100]), {x: 0.5, y: 0.5});

  f = Points.divideFn({x: 100, y: 200});
  t.like(f(50, 100), {x: 0.5, y: 0.5});
  t.like(f({x: 50, y: 100}), {x: 0.5, y: 0.5});
  t.like(f([50, 100]), {x: 0.5, y: 0.5});
  ...
  // Dodgy input
  t.throws(() => Points.divideFn(0, 1));
  t.throws(() => Points.divideFn(1, 0));
  t.throws(() => Points.divideFn(1, 1, 0));
  t.throws(() => Points.divideFn({x: 1, y: 0}));
  t.throws(() => Points.divideFn({x: 0, y: 1}));
  t.throws(() => Points.divideFn(Number.NaN, Number.NaN));

  // Incorrect array length
  t.throws(() => Points.divideFn([]));
  t.throws(() => Points.divideFn([1, 2, 3, 4]));


  const f2 = Points.divideFn({x: 100, y: 200});
