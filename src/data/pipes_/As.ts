...
// export async function asPromise<V>(p: IReadable<V>): Promise<V> {
//   if (p.isClosed) throw new Error(`Pipe has closed`);
...
// export async function* asAsyncIterable<V>(p: IReadable<V>): AsyncGenerator<V> {
...
//  * @returns 
//  */
// export const asArray = async <V>(pipe: IReadable<V>, options: AsArrayOptions = {}): Promise<Array<V>> => {
...
// // export const basic = <V>(p: IReadable<V>, initialFire = false) => {
// //   const t = {
