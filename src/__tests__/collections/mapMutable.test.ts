...
import { mapOfSetMutable } from '../../collections/MapMultiMutable.js';
import { jest } from '@jest/globals';

test(`mapOfSetMutable`, () => {
  const m = mapOfSetMutable<string>();
  ...
  const m2 = mapOfSetMutable<Person>(opts);
...
describe(`mapOfArrayMutable`, () => {
  test(`withOpts`, () => {
    ...
    expect(
      m.hasKeyValue(`Bristol`, { name: `Sally`, city: `Bristol` })
    ).toBeTruthy();
    expect(
      m.hasKeyValue(`London`, { name: `Sally`, city: `Bristol` })
    ).toBeFalsy();
    ...
    expect(m.findKeyForValue({ name: `BARRY`, city: `London` })).toEqual(
      `London`
    );
    expect(
      m.findKeyForValue({ name: `notfound`, city: `notfound` })
    ).toBeUndefined();
  });
