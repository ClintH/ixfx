...
test('pingPong', () => {
  ...
  let given: number[] = []
  let count = 5;
  for (let v of pingPong(10, 10, 100)) {
  ...
  given = []
  count = 5;
  for (let v of pingPong(-10, 10, 100, 100)) {
    expect(v).not.toBeUndefined();
    // @ts-ignore
    given.push(v);
    if (--count == 0) break;
  ...
  given = []
  count = 12;
  for (let v of pingPong(10, 10, 50)) {
    expect(v).not.toBeUndefined();
    // @ts-ignore
    given.push(v);
    if (--count == 0) break;
...
test('pingPongPercent', () => {
  ...
  let given: number[] = []
  let count = 11;
  for (let v of pingPongPercent(0.1)) {
    expect(v).not.toBeUndefined();
    // @ts-ignore
    given.push(v);
    if (--count == 0) break;
  ...
  given = [];
  count = 20;
  for (let v of pingPongPercent(0.2)) {
    expect(v).not.toBeUndefined();
    // @ts-ignore
    given.push(v);
    if (--count == 0) break;
  ...
  given = [];
  count = 5;
  for (let v of pingPongPercent(0.8)) {
    expect(v).not.toBeUndefined();
    // @ts-ignore
    given.push(v);
    if (--count == 0) break;
  ...
  let pp = pingPongPercent(0.1, 0.5);
